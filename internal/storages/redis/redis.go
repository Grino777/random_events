...
func (rs *RedisStorage) SaveProcessedEvent(ctx context.Context, event *models.Event) error {
	...
	default:
		_, err := withClient(rs, func(rc *redis.Client) (models.BrokerMessage, error) {
			...
			if err != nil {
				return models.BrokerMessage{}, fmt.Errorf("%s: %v", op, err)
			}
			...
			if err != nil {
				return models.BrokerMessage{}, fmt.Errorf("failed to save event to hash: %v", err)
			}

			// Сохранение в Set для проверки уникальности
			err = rc.SAdd(ctx, setKey, eventKey).Err()
			if err != nil {
				return models.BrokerMessage{}, fmt.Errorf("failed to save event to set: %v", err)
			}

			// Установка TTL (например, 7 дней) для экономии памяти
			err = rc.Expire(ctx, hashKey, 7*24*time.Hour).Err()
			if err != nil {
				return models.BrokerMessage{}, fmt.Errorf("failed to set TTL for hash: %v", err)
			}
			err = rc.Expire(ctx, setKey, 7*24*time.Hour).Err()
			if err != nil {
				return models.BrokerMessage{}, fmt.Errorf("failed to set TTL for set: %v", err)
			}
